...
- **A masking decision can now be required for every newly added column, instead of the column silently starting to be exported unmasked.** A migration that adds a column leaves `schema:generate` emitting it with no masking, so the next regeneration exports it as-is — before anyone has judged whether it holds personal data. Three pieces make that decision explicit and enforceable. `needs_mask_decision: true` on a column/field marks a masking decision nobody has made yet; extraction ignores the key entirely (what the column exports is whatever `replace_with` / `ignore` say), so it is purely a decision-tracking bit, and the on-disk state wins over regeneration — removing the key is how the decision is recorded. `EXWIW_NEW_COLUMNS=safe rake exwiw:schema:generate` (safe mode; off by default, generated output otherwise unchanged) emits every column the config does not have yet **masked** and carrying that flag, with the default mask following the column type: `masked-{primary key}` for text (with `@example.com` when the column name mentions mail), `0` for numbers, `false` for booleans, a fixed date/timestamp, `{}` for JSON. The primary key and the foreign keys/types the `belongs_tos` join on are flagged but never masked — masking them would break the joins and leave the dump referencing rows that were never exported — and so are types no constant safely fits (`uuid`, `binary`, enums, arrays, text columns too short to hold the masked value), since a default the column cannot hold would fail the restore the dump feeds, which is worse than exporting it while the flag blocks the merge. `rake exwiw:schema:check` reports how the committed config differs from what the application would generate now (added/removed/changed tables and columns) plus the columns still flagged, as sorted JSON with a non-zero exit; it regenerates into a throwaway copy rather than over the config directory, so it runs on a working tree it must not modify — a CI check that keeps a schema change from being merged until the config is reconciled and every new column's masking is decided.
