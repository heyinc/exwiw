...
- **`exwiw:schema:generate` / `schema:tidy` / `schema:generate_mongoid` now read their output directory from the project's `exwiw.yml`.** Previously the generators wrote to `EXWIW_SCHEMA_DIR_PATH` or, failing that, the hard-coded `exwiw/schema` default — so a project that pointed `export`/`explain` at a custom `schema_dir` via the config file still had to repeat that directory in the environment when regenerating. The tasks now resolve the directory with the precedence `EXWIW_SCHEMA_DIR_PATH` (explicit per-run override) → `schema_dir` in `exwiw.yml`/`exwiw.yaml` → `exwiw/schema`, so generation lands in the same directory the CLI reads from without a second source of truth. `schema_dir` is resolved relative to the config file's own directory, exactly as for the CLI, and `EXWIW_CONFIG` may point the tasks at a non-default config path (mirroring `--config`). Callers that already set `EXWIW_SCHEMA_DIR_PATH` are unaffected.
