...
- **`schema:generate` no longer emits a broken cross-database `belongs_to` in multi-database (`connects_to`) setups.** A `belongs_to` whose target model lives in a *different* database (e.g. a `primary` model referencing an `analytics` one) was emitted as a live relation, but its target table is bucketed into a different per-database subdirectory and is therefore absent from the directory the config is loaded with — leaving a dangling edge that crashed dependency resolution at extraction time (a nil-target `NoMethodError`). The generator now detects such relations (by comparing the owning and target models' database config names) and emits them with `ignore: true` and `ignore_type: "cross_database"`, recording why in the `comment`; the relation is dropped from extraction at load time, while the foreign-key column itself is still exported. Polymorphic associations are handled per target, so only the targets that cross a database boundary are ignored. Single-database applications are unaffected. As part of this, `TableConfig#merge` now also preserves a `belongs_to`'s `ignore_type` across regeneration (matching `MongodbCollectionConfig#merge` and the documented contract), so the annotation survives a re-run of `schema:generate`.
