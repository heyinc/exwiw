...
- **MongoDB: cursor-parallel fetch** (experimental, opt-in via the `--cursor-parallel` CLI flag, requires `--parallel-workers=N` with `N>1`, mongodb adapter, `export` only). The plain `--parallel-workers` path only parallelizes Extended-JSON *encoding*; the Mongo cursor's BSON→Ruby *decode* still runs serially in the parent and caps the end-to-end win at ~1.1–1.4×. With this opt-in, each collection is split into `N` disjoint `_id` ranges fetched by separate forked workers (each with its own connection), so the decode is parallelized too — measured byte-identical at ~2.5–5.5× depending on dataset size. The trade-off that makes it strictly opt-in: each per-range cursor sorts by `_id`, so the output is **ordered by `_id` rather than natural collection order** (a different byte stream than the default dump, but a semantically equivalent re-import); the FK-propagation `@state` is republished by merging each worker's per-range capture in range order, so downstream child-collection scoping is unchanged. Omit the flag to keep the byte-identical, natural-order default. Programmatic/Railtie callers can set the `EXWIW_MONGODB_CURSOR_PARALLEL=1` environment variable instead; the CLI flag takes precedence when both are given.
- **MongoDB: parallel document serialization** (`--parallel-workers=N`, mongodb adapter, `export` only). Large or embedded-document-heavy MongoDB dumps are bottlenecked on encoding each document to MongoDB Extended JSON, which is pure Ruby and holds the GVL (so threads give no speedup). Passing `--parallel-workers=N` forks `N` worker processes that serialize contiguous slices of each collection in parallel and the parent concatenates them in order, producing **byte-identical** output to the serial dump. The per-document encoding step parallelizes ~2–2.5× at 4–8 workers, but a real dump also spends a large *serial* fraction decoding the Mongo cursor (BSON→Ruby), so the measured end-to-end speedup is modest (roughly 1.1–1.4× on a 20k-document × 30-embed collection, ~40% of which is the serial decode) and proportional to how embed-heavy the documents are. The trade-off is memory — each worker holds a multi-thousand-document slice, so peak memory grows with `N` — which is why the default (`1`) remains the serial, memory-safe path. Mutually exclusive with the SQL adapters. Programmatic/Railtie callers can set the `EXWIW_MONGODB_PARALLEL_WORKERS` environment variable instead; the CLI flag takes precedence when both are given.
