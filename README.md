...
The schema file carries the source's triggers, so they fire while the `insert-*.sql` files are applied. Disable them for the load if that is not wanted — on PostgreSQL, `SET session_replication_role = 'replica'` (superuser / `rds_superuser`) or `ALTER TABLE ... DISABLE TRIGGER USER`.
