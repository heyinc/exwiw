...
- `--parallel-workers=N` serializes each collection's documents across `N` forked worker processes, for large or embedded-document-heavy dumps. The dump's dominant cost is encoding each document to MongoDB Extended JSON — pure Ruby, so it holds the GVL and threads give no speedup; only separate processes use more cores. The output is byte-identical to the serial dump. The per-document encoding step itself parallelizes ~2–2.5× at 4–8 workers, but a real dump also spends a large, *serial* fraction decoding the Mongo cursor (BSON→Ruby) that parallelism cannot touch — so the **measured end-to-end speedup is modest: roughly 1.1–1.4× on a 20k-document × 30-embed collection** (≈40% of that dump is the serial cursor decode, which caps the win), proportional to how embed-heavy the documents are. Benchmark your own data with `script/bench_mongodb_dump.rb` before relying on it. The trade-off is memory: each worker holds a multi-thousand-document slice, so peak memory grows with `N`. The default (`1`) is the serial, memory-safe path. This flag is **mongodb-only** and applies to `export`. Equivalent for programmatic/Railtie callers: set the `EXWIW_MONGODB_PARALLEL_WORKERS` environment variable (the CLI flag takes precedence when both are given).
- `--cursor-parallel` (experimental, requires `--parallel-workers=N` with `N>1`) goes further than `--parallel-workers` alone. That flag only parallelizes the Extended-JSON *encoding*; the Mongo cursor's BSON→Ruby *decode* still runs serially in the parent and caps the win. With `--cursor-parallel`, each collection is split into `N` disjoint `_id` ranges fetched by separate forked workers (each opening its own connection), so the decode is parallelized too — measured byte-identical at ~2.5–5.5× depending on dataset size (`script/bench_mongodb_cursor_parallel_probe.rb`). The trade-off that keeps it opt-in: each per-range cursor sorts by `_id`, so the **output is ordered by `_id` rather than natural collection order** — a different byte stream than the default dump, but a semantically equivalent re-import. Omit it to keep the byte-identical, natural-order default. This flag is **mongodb-only** and applies to `export`. Equivalent for programmatic/Railtie callers: set the `EXWIW_MONGODB_CURSOR_PARALLEL=1` environment variable (the CLI flag takes precedence when both are given).
