...
- `--parallel-workers=N` serializes each collection's documents across `N` forked worker processes, for large or embedded-document-heavy dumps. The dump's dominant cost is encoding each document to MongoDB Extended JSON — pure Ruby, so it holds the GVL and threads give no speedup; only separate processes use more cores. The output is byte-identical to the serial dump. Expect a sublinear ~2–2.5× speedup at 4–8 workers (it becomes memory-bandwidth/GC bound, not core bound), and the win is proportional to how embed-heavy the documents are. The trade-off is memory: each worker holds a multi-thousand-document slice, so peak memory grows with `N`. The default (`1`) is the serial, memory-safe path. This flag is **mongodb-only** and applies to `export`. Equivalent for programmatic/Railtie callers: set the `EXWIW_MONGODB_PARALLEL_WORKERS` environment variable (the CLI flag takes precedence when both are given).
