...
- **Cursor-parallel fetch** (experimental, opt-in via the `EXWIW_MONGODB_CURSOR_PARALLEL=1` environment variable, requires `--parallel-workers=N` with `N>1`). The default `--parallel-workers` path only parallelizes the Extended-JSON *encoding*; the Mongo cursor's BSON→Ruby *decode* still runs serially in the parent and caps the win. With this opt-in set, each collection is split into `N` disjoint `_id` ranges fetched by separate forked workers (each opening its own connection), so the decode is parallelized too — measured byte-identical at ~2.5–5.5× depending on dataset size (`script/bench_mongodb_cursor_parallel_probe.rb`). The trade-off: each per-range cursor sorts by `_id`, so the **output is ordered by `_id` rather than natural collection order** — a different byte stream than the default dump, but a semantically equivalent re-import. Leave it unset to keep the byte-identical, natural-order default.
